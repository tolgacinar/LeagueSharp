...
namespace Marksman
{
    ...
    internal class Draven : Champion
    {
        ...
        public Draven()
        {
            ...
            Utils.PrintMessage("Draven yuklendi.");
        }
        ...
        {
            config.AddItem(new MenuItem("UseQC", "Q Kullan").SetValue(true));
            config.AddItem(new MenuItem("UseWC", "W Kullan").SetValue(true));
            config.AddItem(new MenuItem("UseEC", "E Kullan").SetValue(true));
            config.AddItem(new MenuItem("UseRC", "R Kullan").SetValue(true));
            return true;
        }

        public override bool HarassMenu(Menu config)
        {
            config.AddItem(new MenuItem("UseQH", "Q Kullan").SetValue(true));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("DrawE", "E menzili").SetValue(new Circle(true, Color.FromArgb(100, 255, 0, 255))));
            config.AddItem(
                new MenuItem("DrawR", "R menzili(2000 adet)").SetValue(
                    new Circle(true, Color.FromArgb(100, 255, 0, 255))));
            config.AddItem(
                new MenuItem("DrawOrbwalk", "Orbwalk pozisyon cizimi").SetValue(new Circle(true, Color.Yellow)));
            config.AddItem(new MenuItem("DrawReticles", "Gorus alani cizimi").SetValue(new Circle(true, Color.Green)));
            return true;
        }
        ...
        public override bool MiscMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("EGapCloser", "E ile uzerine atlamalari kes").SetValue(true));
            config.AddItem(new MenuItem("EInterruptable", "E ile buyuleri kes").SetValue(true));
            config.AddItem(new MenuItem("RManualCast", "Akilli Ulti Manuel(2000 menzil)"))
                .SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press));
            config.AddItem(new MenuItem("Epeel", "E ile kendini koru").SetValue(true));
            config.AddItem(new MenuItem("CatchRadius", "Balta yakalama capi").SetValue(new Slider(600, 200, 1000)));
            return true;
        }
