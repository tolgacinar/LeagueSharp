...
//        public static Menu MenuInterruptableSpell;
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            var targetSelectorMenu = new Menu("Hedef Secici", "Target Selector");
            ...
            /* Menu Summoners */
            var summoners = Config.AddSubMenu(new Menu("Sihirdarlar", "Summoners"));
            var summonersHeal = summoners.AddSubMenu(new Menu("Sifa", "Heal"));
            {
                summonersHeal.AddItem(new MenuItem("SUMHEALENABLE", "Aktif").SetValue(true));
                summonersHeal.AddItem(new MenuItem("SUMHEALSLIDER", "Min. Can").SetValue(new Slider(20, 99, 1)));
            }

            var summonersBarrier = summoners.AddSubMenu(new Menu("Bariyer", "Barrier"));
            {
                summonersBarrier.AddItem(new MenuItem("SUMBARRIERENABLE", "Aktif").SetValue(true));
                summonersBarrier.AddItem(
                    new MenuItem("SUMBARRIERSLIDER", "Min. Can").SetValue(new Slider(20, 99, 1)));
            }

            var summonersIgnite = summoners.AddSubMenu(new Menu("Tutustur", "Ignite"));
            {
                summonersIgnite.AddItem(new MenuItem("SUMIGNITEENABLE", "Aktif").SetValue(true));
            }
            /* Menu Items */            
            var items = Config.AddSubMenu(new Menu("Esyalar", "Items"));
            items.AddItem(new MenuItem("BOTRK", "Mahv. Kralin Kilici").SetValue(true));
            items.AddItem(new MenuItem("GHOSTBLADE", "Youmu").SetValue(true));
            items.AddItem(new MenuItem("SWORD", "Hirs Kilici").SetValue(true));
            items.AddItem(new MenuItem("MURAMANA", "Muramana").SetValue(true));
            QuickSilverMenu = new Menu("QSS", "Civa");
            ...
            items.AddItem(
                new MenuItem("UseItemsMode", "Esya kullanma acik").SetValue(
                    new StringList(new[] {"Hayir", "Karisik mod", "Kombo Modu", "Ikiside"}, 2)));
            ...
            {
                var combo = new Menu("Kombo", "Combo");
                if (CClass.ComboMenu(combo))
                {
                    Config.AddSubMenu(combo);
                }

                var harass = new Menu("Haras", "Harass");
                if (CClass.HarassMenu(harass))
                {
                    harass.AddItem(new MenuItem("HarassMana", "Minimum mana yuzdesi").SetValue(new Slider(50, 100, 0)));
                    Config.AddSubMenu(harass);
                }

                var laneclear = new Menu("Koridor Temizleme", "LaneClear");
                if (CClass.LaneClearMenu(laneclear))
                {
                    laneclear.AddItem(
                        new MenuItem("LaneClearMana", "Minimum mana yuzdesi").SetValue(new Slider(50, 100, 0)));
                    Config.AddSubMenu(laneclear);
                }

                var misc = new Menu("Cesitli", "Misc");
                ...
                */
                var extras = new Menu("Ekstralar", "Extras");
                ...
                var drawing = new Menu("Cizimler", "Drawings");
                if (CClass.DrawingMenu(drawing))
                {
                    drawing.AddItem(
                        new MenuItem("drawMinionLastHit", "Minyon Son Vurus").SetValue(new Circle(false,
                            System.Drawing.Color.GreenYellow)));
                    drawing.AddItem(
                        new MenuItem("drawMinionNearKill", "Yakindaki minyonu oldur").SetValue(new Circle(false,
