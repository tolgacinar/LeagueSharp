...
namespace Marksman
{
    internal class Varus : Champion
    {
        ...
        public Varus()
        {
            Utils.PrintMessage("Varus yüklendi!");
        ...
        {
            config.AddItem(
                new MenuItem("UseQC" + Id, "Q Modu").SetValue(new StringList(new[] {"Kapali", "Her zaman kullan", "Stack 3 oldugunda"}, 0)));
            config.AddItem(new MenuItem("UseEC" + Id, "E kullan").SetValue(true));
            config.AddItem(new MenuItem("UseRC" + Id, "Ulti kullan").SetValue(true));

            config.AddItem(
                new MenuItem("QMinChargeC", "Minimum Q Sarji").SetValue(new Slider(1000, Q.ChargedMinRange,
                    Q.ChargedMaxRange)));
            config.AddItem(
                new MenuItem("UseQ2C" + Id, "Once Q Kullan ").SetValue(new KeyBind("J".ToCharArray()[0], KeyBindType.Press)));
            return true;
        }
        ...
        public override bool HarassMenu(Menu config)
        {
            config.AddItem(
                new MenuItem("UseQH" + Id, "Q").SetValue(new StringList(
                    new[] {"Kapali", "Her zaman kullan", "Stack 3 oldugunda"}, 0)));
            config.AddItem(new MenuItem("UseEH" + Id, "E").SetValue(true));
            config.AddItem(
                new MenuItem("UseETH" + Id, "E kullan (Gecis)").SetValue(new KeyBind("H".ToCharArray()[0],
                    KeyBindType.Toggle)));
        ...
        {
            config.AddItem(new MenuItem("spellDelay" + Id, "Buyu gecikmesi").SetValue(new Slider(500, 0, 3000)));
            config.AddItem(
                new MenuItem("CastR" + Id, "Akilli Ulti").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("DrawQC" + Id, "Min. Q Sarji").SetValue(new Circle(true, Color.White)));
            config.AddItem(new MenuItem("DrawRS" + Id, "R: Alan Taraniyor...").SetValue(new Circle(true, Color.White)));
