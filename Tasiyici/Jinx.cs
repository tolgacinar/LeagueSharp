...
namespace Marksman
{
    internal class Jinx : Champion
    {
        ...
        public Jinx()
        {
            ...
            Utils.PrintMessage("Jinx yuklendi.");
        }
        ...
        {
            config.AddItem(new MenuItem("UseQC" + Id, "Q Kullan").SetValue(true));
            config.AddItem(new MenuItem("UseWC" + Id, "W Kullan").SetValue(true));
            config.AddItem(new MenuItem("UseRC" + Id, "R Kullan").SetValue(true));
            return true;
        }

        public override bool HarassMenu(Menu config)
        {
            config.AddItem(new MenuItem("UseQH" + Id, "Q Kullan").SetValue(true));
            config.AddItem(new MenuItem("UseWH" + Id, "W Kullan").SetValue(false));
            return true;
        }

        public override bool LaneClearMenu(Menu config)
        {
            config.AddItem(new MenuItem("SwapQ" + Id, "Her Zaman Minigun Kullan").SetValue(false));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("SwapDistance" + Id, "Menzil icin Q kullan").SetValue(true));
            config.AddItem(new MenuItem("SwapAOE" + Id, "AOE icin Q kullan").SetValue(false));
            config.AddItem(new MenuItem("MinWRange" + Id, "Min W menzili").SetValue(new Slider(525 + 65 * 2, 0, 1200)));
            config.AddItem(new MenuItem("AutoEI" + Id, "Hareketsiz birakmak icin E kullan").SetValue(true));
            config.AddItem(new MenuItem("AutoES" + Id, "Yavaslatmak icin E kullan").SetValue(true));
            config.AddItem(new MenuItem("AutoED" + Id, "Gosteris icin E kullan").SetValue(false));
            config.AddItem(
                new MenuItem("CastR" + Id, "Akilli Ulti (2000 Menzil)").SetValue(
                    new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            config.AddItem(new MenuItem("ROverKill" + Id, "Ulti icin cani kontrol et").SetValue(true));
            config.AddItem(new MenuItem("MinRRange" + Id, "Min R menzili").SetValue(new Slider(300, 0, 1500)));
            config.AddItem(new MenuItem("MaxRRange" + Id, "Max R menzili").SetValue(new Slider(1700, 0, 4000)));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("DrawQBound" + Id, "Q Cizimi").SetValue(
                    new Circle(true, Color.FromArgb(100, 255, 0, 0))));
            config.AddItem(
                new MenuItem("DrawW" + Id, "W mezili").SetValue(new Circle(false, Color.CornflowerBlue)));
            return true;
        }
