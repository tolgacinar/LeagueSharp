...
namespace Marksman
{
    internal class Vayne : Champion
    {
        ...
        public Vayne()
        {
            ...
            Utils.PrintMessage("Vayne yüklendi");
        }
        ...
        {
            config.AddItem(
                new MenuItem("UseET" + Id, "E kullan (Gecis)").SetValue(
                    new KeyBind("T".ToCharArray()[0], KeyBindType.Toggle)));
            config.AddItem(new MenuItem("UseEInterrupt" + Id, "E ile Engellemek").SetValue(true));
            config.AddItem(new MenuItem("PushDistance" + Id, "E ittirme mesafesi").SetValue(new Slider(425, 475, 300)));
            config.AddItem(new MenuItem("CompleteSilverBuff" + Id, "Pasifi Q ile bitir").SetValue(true));
            return true;
        }
